...
/*
    image purification
*/
inline void applyThreshold(cv::Mat& org, cv::Mat& mod) {
...
*/
inline double getAngle(cv::Point p1, cv::Point p2, cv::Point p3) {
...
*/
inline bool isValidRect(std::vector<cv::Point>& contour) {
...
*/
inline std::vector<cv::Point> findSudokuGrid(cv::Mat& mod) {
...
*/
inline std::vector<cv::Point> getSudokuCorners(std::vector<cv::Point>& grid) {
...
*/
inline std::pair<cv::Mat, cv::Mat> transformPerspective(cv::Mat& org, std::vector<cv::Point>& srcCorners) {
...
*/
inline std::vector<cv::Mat> split(cv::Mat& org, int m, int n) {
